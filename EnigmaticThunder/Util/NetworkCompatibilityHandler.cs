    ...
    internal class NetworkCompatibilityHandler
    {
        ...
        private void ScanPluginsForNetworkCompat(object _, EventArgs __)
        {
            foreach (var (_, pluginInfo) in BepInEx.Bootstrap.Chainloader.PluginInfos)
            {
                ...
                {
                    LogCore.LogE($"Exception in ScanPluginsForNetworkCompat while scanning plugin {pluginInfo.Metadata.GUID}");
                    LogCore.LogE("EnigmaticThunder Failed to properly scan the assembly." + Environment.NewLine +
...
#pragma warning restore CS8605 // Unboxing a possibly null value.
        ...
        private void AddToNetworkModList()
        {
            if (ModList.Count != 0)
            {
                //There's no type of networking API so like
                //yeah
